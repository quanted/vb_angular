...
export class CreatePipelineComponent implements OnInit {
  ...
  estimatorChange(e): void {
    ...
    this.pipelineFormGroup.controls.pipelineNameCtrl.setValue(
      this.selectedPipeline.name
    );
    this.pipelineFormGroup.controls.pipelineDescCtrl.setValue(
      this.selectedPipeline.description
    );
   ...
   */
  addPipeline() {
    const hyperParams = this.pipelineFormGroup.controls
      .hyperParamCtrl as FormArray;
    ...
    const newPipeline = {
      ...
      type: this.pipelineInfo.find((pipeline) => {
        return (
          this.pipelineFormGroup.controls.estimatorCtrl.value === pipeline.name
        );
      }).ptype,
    ...
    this.pipelineService.updatePipeline(this.vbHelper).subscribe((res) => {
      console.log("updatePipeline: ", res);
